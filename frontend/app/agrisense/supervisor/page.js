...
import { supervisorAPI } from "../agrisense-api";
...
// ── Navbar ─────────────────────────────────────────────────────
function SupervisorNav({ session, crumbs }) {
  const router = useRouter();
  return (
    <nav className="sup-navbar">
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        <div className="sup-nav-brand" onClick={() => router.push("/agrisense/supervisor")} style={{ cursor: "pointer" }}>
          ...
          <div className="sup-nav-crumb">
            {crumbs.map((c, i) => (
              <span key={i} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <span style={{ color: "#d9e8de" }}>›</span>
                {c.href
                  ? <span onClick={() => router.push(c.href)} style={{ color: "#4d6659", cursor: "pointer" }}>{c.label}</span>
                  : <span style={{ color: "#1a2e25", fontWeight: 600 }}>{c.label}</span>
                }
              </span>
            ))}
          </div>
      ...
      <div className="sup-nav-user">
        <div className="sup-nav-user-info">
          <span className="sup-nav-user-name">{session?.user?.name}</span>
          <span className="sup-nav-role-badge">Supervisor</span>
        </div>
        <Link href="/dashboard" className="agri-back-to-florisight" style={{ marginRight: 8 }}>← FloriSight</Link>
        <button
...
// ── Main Page ──────────────────────────────────────────────────
export default function AgriSenseSupervisorPage() {
            ...
            <div className="sup-farmer-list" style={{ marginTop: 16 }}>
              {farmers.map(farmer => (
                <div
                  ...
                  onClick={() => router.push(`/agrisense/supervisor/farmer/${farmer.id}`)}
                  role="button"
                  tabIndex={0}
                >
                  ...
                  <button
                    ...
                    onClick={e => { e.stopPropagation(); router.push(`/agrisense/supervisor/farmer/${farmer.id}`); }}
                    tabIndex={-1}
                  >
                    View Profile →
                  </button>
        ...
        <div className="sup-section-block">
          ...
          {!farmersLoading && allFarmers.length > 0 && (
            <>
              ...
              {hasMore && (
                <button className="sup-expand-btn" onClick={() => setFarmersExpanded(v => !v)} id="btn-farmers-expand">
              ...
              {myVisits.map(v => {
                ...
                return (
                  <div
                    ...
                    onClick={() => router.push(`/agrisense/supervisor/farmer/${v.farmer_id}/farm/${v.farm_id}`)}
                    role="button"
                    tabIndex={0}
                  >
